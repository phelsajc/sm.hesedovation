...
{
    public function courseQuick(Request $request,$id)
    {
    ...
    {    	
        DB::table('courses')->where('id','=',$id)->update(['status' => "6"]);	
        return back()->with('success','Status changed to Reject !');
    }

    public function courseQuickLive($id)
    {    	
        DB::table('courses')->where('id','=',$id)->update(['status' => "7"]);	
        return back()->with('success','Status changed to Live !');
    }
    ...
    public function orderQuick($id)
    {
        ...
        $courseDetail = Course::where(['id'=>$order->course_id])->first(); */
        //echo $userDetail->email;exit;
        if($order->status == 0)
        {
            DB::table('orders')->where('id','=',$id)->update(['status' => "1"]);   
            /* $user = array(
                'fname'=> $userDetail->fname.' '.$userDetail->lname,
                'course'=> $courseDetail->title
            ); */
            //Mail::to($userDetail->email)->send(new CourseOrder($user));
            return back()->with('success','Status changed to Active !');
        }
        else
        {
