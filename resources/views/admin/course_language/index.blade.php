                    ...
                    <tr>
                      ...
                      <td>
                        <form action="{{ route('language.quick',$cat->id) }}" method="POST">
                          {{ csrf_field() }}
                          <button type="Submit" class="btn btn-xs {{ $cat->status ==1 ? 'btn-success' : 'btn-danger' }}" title="{{ $cat->status ==1 ? 'Active' : 'Inactive' }}">
                            @if($cat->status ==1)
                      ...
                      <td>
                        ...
                        @if(Auth::user()->role == "admin")
                          <a class="btn btn-success btn-sm" title="Edit" href="{{url('courselang/'.$cat->id.'/edit')}}"><i class="glyphicon glyphicon-pencil"></i></a>
                        @else
                          <a type="submit" class="btn btn-success btn-sm" data-toggle="tooltip" rel="tooltip" data-placement="top" disabled="disabled" title="Edit Disabled"><i class="glyphicon glyphicon-pencil"></i></a>
                        @endif
                        ...
                        @else
                          <a type="submit" class="btn btn-danger btn-sm" data-toggle="tooltip" rel="tooltip" data-placement="top" disabled="disabled" title="Delete Disabled"><i class="fa fa-fw fa-trash-o"></i></a>
                        @endif
                      </td>
                      ...
                      {{-- <td>
                        @if(Auth::user()->role == "admin")
                          <form method="post" action="{{url('courselang/'.$cat->id)}}
                              ...
                              {{ csrf_field() }}
                              {{ method_field('DELETE') }}
                              <button  type="submit" class="btn btn-danger" title="Delete"><i class="fa fa-fw fa-trash-o"></i></button>
                          </form>
                        @else
                          <a type="submit" class="btn btn-danger btn-sm" data-toggle="tooltip" rel="tooltip" data-placement="top" disabled="disabled" title="Delete Disabled"><i class="fa fa-fw fa-trash-o"></i></a>
                        @endif
